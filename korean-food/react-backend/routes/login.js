...
router.post("/", async function(req, res, next) {
  ...
  const derivedKey = pbkdf2.pbkdf2Sync(password, 'NoRainbowTableAttacks!', 1, 32, 'sha512').toString('base64');
  ...
  if(loginResponse === ""){
	...
	// Generate a random token. Fix vulnerability from https://github.com/mdulin2/SMC2/blob/master/korean-food/Solution.md
        var token = crypto.randomBytes(64).toString('hex'); 
	const cookie = await auth.createSession(username, token);
